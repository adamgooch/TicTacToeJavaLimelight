...
public class IOTest extends TestSuite {
    ...
    @Before
    public void setUp() {
        GameMaker.board = mock(Board.class);
        io = new ConsoleIO();
    }
    ...
    public void getValidPlayerMoveShouldBeTrueWhenThePlayerEnters4() {
        createUserInput("4\n");
        when(GameMaker.board.putMarkInSquare('X', 4)).thenReturn(true);
    ...
    public void getValidPlayerMoveShouldReturnTrueWhenThePlayerEnters8() {
        createUserInput("8\n");
        when(GameMaker.board.putMarkInSquare('X', 8)).thenReturn(true);
        assertTrue(io.isValidPlayerMove('X'));
    }
    ...
    public void getValidPlayerMoveShouldReturnFalseWhenThePlayerEnters9() {
        createUserInput("9\n");
        when(GameMaker.board.putMarkInSquare('X', 9)).thenReturn(false);
